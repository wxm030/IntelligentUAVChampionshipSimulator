...
void AirsimROSWrapper::create_ros_pubs_from_settings_json()
{
        ...
        for (auto& curr_camera_elem : vehicle_setting->cameras) {
            ...
            for (const auto& curr_capture_elem : camera_setting.capture_settings) {
                ...
                if (!(std::isnan(capture_setting.fov_degrees))) {
                    ...
                    if(strcmp(curr_camera_name.c_str(), "bottom_center")==0)
                    {
                        bottom_pub_ = image_transporter.advertise(cam_image_topic, 1);
                        bottom_request_.push_back(ImageRequest(curr_camera_name, curr_image_type, false, false));
                    ...
                    if(strcmp(curr_camera_name.c_str(), "front_center")==0)
                    {
                        ...
                        if(curr_image_type == ImageType::Scene)
                        {
                            ...
                            RGBD_request_.push_back(ImageRequest(curr_camera_name, curr_image_type, false, false));
                        ...
                        }else
                        {
                            ...
                            RGBD_request_.push_back(ImageRequest(curr_camera_name, curr_image_type, true, false));
    ...
    if (!airsim_img_request_vehicle_name_pair_vec_.empty()) {
        ...
        if(is_RGBD_)
        {
            ros::TimerOptions timer_options_RGBD(ros::Duration(update_airsim_img_response_every_n_sec),
                                        boost::bind(&AirsimROSWrapper::img_response_RGBD_timer_cb, this, _1),
                                        &img_timer_cb_queue_RGBD_);
            airsim_img_response_RGBD_timer_ = nh_private_.createTimer(timer_options_RGBD);
        }
        
        if(is_stereo_)
        {
            ros::TimerOptions timer_options_stereo(ros::Duration(update_airsim_img_response_every_n_sec),
                                boost::bind(&AirsimROSWrapper::img_response_stereo_timer_cb, this, _1),
                                &img_timer_cb_queue_stereo_);

            airsim_img_response_stereo_timer_ = nh_private_.createTimer(timer_options_stereo);
...
void AirsimROSWrapper::update_commands()
{
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        ...
        if (airsim_mode_ == AIRSIM_MODE::DRONE) {
            ...
            if(drone->has_angle_rate_throttle_cmd) {
                std::lock_guard<std::mutex> guard(drone_control_mutex_);
                get_multirotor_client()->moveByAngleRatesThrottleAsync(drone->angle_rate_throttle_cmd.rollRate,
                                                                       ...
                                                                       vel_cmd_duration_,
                                                                       drone->vehicle_name);
                drone->has_angle_rate_throttle_cmd = false;
            }
...
void AirsimROSWrapper::img_response_RGBD_timer_cb(const ros::TimerEvent& event)
{
    try {
        const std::vector<ImageResponse>& img_response = airsim_client_images_.simGetImages(RGBD_request_, airsim_img_request_vehicle_name_pair_vec_[0].second);
...
void AirsimROSWrapper::img_response_bottom_timer_cb(const ros::TimerEvent& event)
{
    try {
        const std::vector<ImageResponse>& img_response = airsim_client_images_.simGetImages(bottom_request_, airsim_img_request_vehicle_name_pair_vec_[0].second);
        if (img_response.size() == bottom_request_.size()) {
            ros::Time curr_ros_time = ros::Time::now();

            for (const auto& curr_img_response : img_response) {
                bottom_camera_info_.header.stamp =  airsim_timestamp_to_ros(curr_img_response.time_stamp);
                bottom_cam_info_pub_.publish(bottom_camera_info_);
                bottom_pub_.publish(get_img_msg_from_response(curr_img_response,
                                                                        curr_ros_time,
                                                                        curr_img_response.camera_name + "_optical"));
            }
        }
